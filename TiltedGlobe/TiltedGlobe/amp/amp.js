...
Module.prototype.evaluatePaths = function(path) {
  var paths;
  paths = this.config.paths || this.parentModule.config.paths;
...
SeekCommand.prototype.execute = function(notification) {
  var app, handler, mediaProxy, playbackProxy, time, video;
  ...
  if (time === playbackProxy.getCurrentTime()) {
    return;
  }
  ...
  app.pausedBeforeSeek = video.paused;
...
SeekedCommand.prototype.execute = function(notification) {
  var app, device, state;
  app = this.facade.retrieveProxy(ApplicationStateProxy.NAME);
  app.setSeeking(false);
  device = Utils.getDevice();
  if (device === "desktop" || device === "android") {
    state = app.pausedBeforeSeek === true ? PlayState.PAUSED : PlayState.PLAYING;
...
var AISNotifications = {
  START_AUTHENTICATION: "authenticationstarted",
  AUTHENTICATED: "authenticated",
  AUTHENTICATING: "authenticating",
  AUTHENTICATION_COMPLETE: "authenticationcomplete",
  CHOOSE_AUTH_PROVIDER: "chooseprovider",
...
FlashPlayer.prototype.createXML = function(config) {
  var app, element, elements, key, mode, node, parent, plugin, plugins, str, value, xml, _i, _j, _len, _len1, _ref, _ref1, _ref2;
  ...
  plugins = ((_ref1 = config.flash["static"]) != null ? _ref1.enabled : void 0) === true ? config.flash["static"].plugins : config.flash.plugins;
